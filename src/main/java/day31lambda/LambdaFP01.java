...
public class LambdaFP01 {
    ...
    /*
     ...
     thousands of times before it was deployed. Function programming can be used for arrays, collections and maps
     but we cannot use maps directly in functional programming directly. At the beginning we should convert a map to a
     collection and then we can use maps for functional programming as wheel ==>How to convert a map into a collection??
      ...
      2) In functional programming we focus on "how to do?" but in structure programming language we focus on "what to do?"
      3) Functional programming can be used just with arrays and collections.
      4) By using "entrySet()" method java convert MAP to set than you can use functional programming in Maps as well.

      //Java will take the elements one by one with the help of stream() method
//And then eliminate repeated ones by the help of distinct() method
...
//And then return  the greater value with the help of Ternary Structure
    ...
    //2) Create a method to print the even elements in the list on the consol in the same line with space
    //bwtween two consecutive elements (Structure P)
    ...
    public static void getMinElement03(List<Integer> list){
        Integer min = list.stream().distinct().sorted().reduce(Integer.MAX_VALUE, (t,u)-> t>u ? u : t );
        System.out.print(min);
    }

    //2.way

    public static void getMinElement04(List<Integer> list){
